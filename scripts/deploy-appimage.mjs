 ...
 * ~/Games/CNGM (two AppImages, no config) while the real 930-game library sat in
 * ~/CafeNeurotico next to yesterday's build.
 *
 ...
 *
 * ⚠️ Machine-specific paths. CAFENEUROTICO_DEPLOY_DIR overrides everything, which is the
...
const FILE = 'CafeNeurotico.AppImage';
const BUILT = path.resolve('dist', FILE);

const CANDIDATES = [
    process.env.CAFENEUROTICO_DEPLOY_DIR,
    path.join(HOME, 'CafeNeurotico'),
...
try {
    ...
    if (fs.existsSync(dest)) fs.renameSync(dest, path.join(target, 'CafeNeurotico_old.AppImage'));
