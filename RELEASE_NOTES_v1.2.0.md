...
- **Steam** is flagged only. Cafe Neurotico reads Steam's own local files to see that an update is waiting and offers **Open in Steam** — it can't query Valve's servers, so it never pretends to update Steam games itself.
...
- Scan for Updates and Add to Desktop are desktop-face features (The Manager); CREMA is unchanged.
